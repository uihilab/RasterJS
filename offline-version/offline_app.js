...
var selectionPaneIdPrefix = "selectionPane",
  imageLayerIdPrefix = "imageLayer",
  watershedLayerIdPrefix = "watershedLayer";
...
function handleDownEvent(evt) {
  ...
  var feature = map.forEachFeatureAtPixel(evt.pixel, function (feature) {
    if (feature.id_ == selectionPaneIdPrefix + currSelectionPaneId)
      return feature;
  });
...
function init() {
  ...
  tileLayer = new ol.layer.Tile({
    extent: [
      -92.31333366538001, 41.29734244973272, -90.22730461264564,
      42.33280387551397,
    ],
...
window.show_area = function show_area() {
  ...
  map.getLayers().forEach((layer) => {
    if (
      layer &&
      layer.get("name") === selectionPaneIdPrefix + currSelectionPaneId
    ) {
...
window.toggle_data = function toggle_data() {
  map.getLayers().forEach((layer) => {
    if (
      layer &&
      layer.get("name") === imageLayerIdPrefix + (currImageLayerId - 1)
    ) {
      if (layer.getVisible()) {
...
window.load_data = function load_data(rasterjs) {
  ...
  map.getLayers().forEach((layer) => {
    if (
      ...
      let selectionPaneFeature = layer
        .getSource()
        .getFeatureById(selectionPaneIdPrefix + currSelectionPaneId);
      ...
      if (rasterjs) {
        ...
        rasterApi
          ...
          .then((dataURL) => {
            let minLat = Math.min(
              selectionPaneCoordinates[0][0][1].toFixed(6),
              selectionPaneCoordinates[0][1][1].toFixed(6),
              selectionPaneCoordinates[0][2][1].toFixed(6),
              selectionPaneCoordinates[0][3][1].toFixed(6)
            );

            let minLong = Math.min(
              selectionPaneCoordinates[0][0][0].toFixed(6),
              selectionPaneCoordinates[0][1][0].toFixed(6),
              selectionPaneCoordinates[0][2][0].toFixed(6),
              selectionPaneCoordinates[0][3][0].toFixed(6)
            );
            ...
            map.getLayers().forEach((layer) => {
              ...
              if (
                layer &&
                layer.get("name") ===
                  selectionPaneIdPrefix + currSelectionPaneId
              ) {
              ...
              if (
                layer &&
                layer.get("name") ===
                  imageLayerIdPrefix + (currImageLayerId - 1)
              ) {
                layersToRemove.push(layer);
              }
              ...
              if (
                layer &&
                layer.get("name") ===
                  watershedLayerIdPrefix + (currWaterShedLayerId - 1)
              ) {
                layersToRemove.push(layer);
              }
            });
          ...
          .catch((error) => {
            console.log(error);
            alert(
              "Raster for the requested area is currently unavailable in offline mode."
            );
            map.getLayers().forEach((layer) => {
              if (
                layer &&
...
function delinate_watershed(lat, lng) {
  ...
  // update current selection pane coordinates
  // if image is not yet projected
  map.getLayers().forEach((layer) => {
    if (
      layer &&
      layer.get("name") === selectionPaneIdPrefix + currSelectionPaneId
...
function draw_watershed(x0, y0, dx, dy, w, h, points, numcells) {
  ...
  map.getLayers().forEach((layer) => {
    if (
      layer &&
      layer.get("name") === watershedLayerIdPrefix + (currWaterShedLayerId - 1)
    ) {
      map.removeLayer(layer);
    }
  });
