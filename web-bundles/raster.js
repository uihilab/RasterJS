...
export default class Raster {
     ...
     * Merges the images specified in indexes in one canvas and extracts the desired image from that canvas according to given
     * x,y pixel coordinate in position object
     *
     * @param {array} indexes index objects with row and column number property of images in the region of interest.
    ...
    getImage(indexes, roiDimension, position, resolution, elevation, area) {
        return this._getSelectedImageDataUrl(indexes, roiDimension, position, resolution, elevation, area)
    }

    _getSelectedImageDataUrl(indexes, roiDimension, position, resolution, elevation, area) {
        return new Promise((resolve, reject) => {
            RasterBusiness.getImage(indexes, roiDimension, 
                this._getResInWord(resolution), elevation).then((canvas) => {
                canvas.id = "canvas"
                resolve(this._getDesiredImageDataUrl(canvas, resolution, position, area))
    ...
    _getDesiredImageDataUrl(canvas, resolution, position, dimension) {
        ...
        if (position['x'] >= 0 && position['y'] >= 0) {
            ...
            // cutting image
            subImageCanvas.getContext('2d').drawImage(canvas, position['x'], position['y'],
            subImageCanvas.width, subImageCanvas.height, 0, 0, subImageCanvas.width, subImageCanvas.height);
