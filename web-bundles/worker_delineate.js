onmessage = function(e) {
	var arr = instant_watershed(e.data[0], e.data[1], e.data[2], e.data[3], e.data[4], e.data[5]);
	postMessage(arr);
  }
  ...
  function instant_watershed(x, y, w, h, flowdir, toolType) {
	  ...
	  if (toolType == "watershed")
		  return find_watershed(x, y, w, h, flowdir);
	  ...
	  function find_drainagepath(x, y, w, h, flowdir) {
		  ...
		  // var dirf = new Int32Array([0,-1,0,1,-1,0,1,-1,0,1]);
		  // var dirg = new Int32Array([0,1,1,1,0,0,0,-1,-1,-1]);
		  ...
		  //console.log((t52-t51)+'ms / Len: '+newpro.length);
		  //draw_rain_tracker(newpro);
	  ...
	  function find_watershed(x, y, w, h, flowdir) {
		  ...
		  matrix[x+w*y] = 1|0;
		  var j=1|0;
		  // var dirf = new Int32Array([-1,0,1,-1,1,-1,0,1]); // x
		  // var dirg = new Int32Array([1,1,1,0,0,-1,-1,-1]); // y
