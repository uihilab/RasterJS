...
function labelFlats(x, y, label, labels, toFill, tmpQueue) {
  // np
  toFill[0] = 1;
  ...
  while (toFill[0] > 0) {
    ...
    for (var k = 0; k < toFill[0]; k++) {
      for (var i = 0; i < 8; i++) {
        ...
        if (
          outsideGrid(newX, newY) ||
          labels[pos_XY] != 0 ||
          dem[pos_XY] != elevation
        )
          continue;
        labels[pos_XY] = label;
        tmpQueue[0]++;
...
//apply a gradient towards lower areas on flats.
// 2 times more weight than awayFromHigher.
  ...
  while (lowEdges[0] > 0) {
    ...
    for (var k = 0; k < lowEdges[0]; k++) {
      ...
      if (flatMask[pos_xy] < 0) {
        flatMask[pos_xy] =
          flatHeight[labels[pos_xy]] + flatMask[pos_xy] + 2 * loops;
      } else {
...
function resolveFlats() {
  ...
  if (lowEdges[0] === 0) {
    if (highEdges[0] === 0) {
      ...
      console.log(
        "undrainable flats -- should have been covered by the depression flooding algorithm?"
      );
    }
    return;
  }
  ...
  if (removed) {
    console.log(
      "not all flats have outlets -- should never happen b/c depression filling"
    );
  }
...
function d8(x, y) {
  ...
  for (var i = 0; i < 8; i++) {
    ...
    var divide =
      moveX[i] + moveY[i] === 0 || Math.abs(moveX[i] + moveY[i]) === 2;
...
function priorityFlood() {
  ...
  while (!open.isEmpty() || pit.length > 0) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      if (outsideGrid(newX, newY) || checked[newX * m + newY]) continue;
...
//gets the accumulated area over
// the current cell and its parents.
...
//gets the nearest drainage cell and computes the
// HAND for each cell which drains into the current one.
function computeNearestDrainageCells(area) {
  ...
  function getNearestDrainageCells(x, y, closestCellX, closestCellY) {
    ...
    if (area[pos] >= drainageThreshold) {
      ...
      hand[pos] =
        originalDEM[pos] - originalDEM[closestCellX * m + closestCellY];
      for (var i = 0; i < parents[pos * 8]; i++) {
        var p = parents[pos * 8 + i + 1];
        var d2 = get2d(p);
        var newx = d2[0];
...
this.onmessage = function (e) {
  if (e.data.data !== undefined) {
    ...
    this.postMessage({
      result: { Hand: hand, Area: area, Parents: parents, FlowDir: flowDir },
    });
